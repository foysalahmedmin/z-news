...
// `components/(common)/news-articles-page/NewsArticlesWorkflowKanbanSection`.
const COLUMNS = [
                ...
                <CardContent className="p-3">
                  ...
                  <div className="text-muted-foreground flex items-center justify-between text-[11px]">
                    <div className="flex items-center gap-1">
                      ...
                      {w.deadline
                        ? // `deadline` is typed `Date` in workflow.service.ts but the
                          // API actually returns an ISO string; `String()` normalizes
                          // either shape for the `Date` constructor.
                          new Date(String(w.deadline)).toLocaleDateString()
                        : "No deadline"}
                    </div>
