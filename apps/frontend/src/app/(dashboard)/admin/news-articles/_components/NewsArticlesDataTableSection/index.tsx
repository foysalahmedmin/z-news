...
// `components/(common)/news-articles-page/NewsArticlesDataTableSection`.
// The original also threaded a `breadcrumbs` prop into react-router's
// `<Link state={...}>` for the view/edit links; Next.js's `<Link>` has no
// history-state equivalent and the destination pages never read it, so it's
// dropped here (same call made porting EventsDataTableSection).
          ...
          {(["super-admin", "admin", "editor"].includes(info?.role || "") ||
            ...
            <Link
              href={
                info?.role !== "admin" && row.author?._id !== info?._id
                  ? "#"
                  : `/admin/news-articles/edit/${row._id}`
              }
            >
              <Button
                className={cn("", {
