...
// `components/(common)/news-articles-details-page/NewsArticleInfoSection`.
// Links to the public site (ENV.app_url) use plain `<a>` tags rather than
// next/link's `<Link>`, since that's a separate app/deployment outside this
// Next.js app's router.
...
const NewsArticleInfoSection: React.FC<NewsInfoSectionProps> = ({ news }) => {
  return (
    <div className="flex items-start space-x-4">
      {/* Thumbnail */}
      <a href={ENV.app_url + "/news/" + news?.slug} className="flex-shrink-0">
        <img
      ...
      <div className="min-w-0 flex-1 space-y-2 space-x-2">
        ...
        <div className="flex items-center space-x-2">
          <a
            href={ENV.app_url + "/news/" + news?.slug}
        ...
        <div>
          {news?.published_at && (
            <span className="flex items-center">
              ...
              {new Date(String(news.published_at)).toLocaleString("en-US", {
                month: "long",
                day: "numeric",
                year: "numeric",
              })}
