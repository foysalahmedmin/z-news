...
// `components/(common)/users-page/UsersDataTableSection`. The original also
// accepted a `breadcrumbs` prop, used only to stash a react-router `state`
// object on the "view" Link for the destination page to read back — Next's
// `Link` has no `state` prop and the destination (`/admin/users/[id]`)
// re-fetches by id instead, so it's dropped here (same dead-prop pattern as
// apps/frontend's already-ported EventsDataTableSection).
