...
// Ported from apps/adminpanel's src/pages/(common)/BinPage/index.tsx.
//
// Route roles: ["super-admin", "admin"] — enforced by middleware.ts, which
// derives per-route role requirements from admin-menu-items.ts's tree (see
// getRequiredRoles there), so no page-level guard is needed here.
//
// Each tab intentionally uses its own admin-<domain>.service.ts (not the
// public read-only services) since it operates on soft-deleted rows via
// `is_deleted: true` plus restore/permanent-delete endpoints that only
// exist on the admin surface.
