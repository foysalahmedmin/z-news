...
// `components/(common)/files-page/FilesDataTableSection`. react-router's
// `Link` (with a `state` payload carrying breadcrumbs for the destination
// page) -> `next/link`'s `Link`, which has no equivalent `state` prop; the
// view page builds its own breadcrumbs instead. The `breadcrumbs` prop and
// `onAdd` prop from the original were never read for anything other than
// that dropped `state` payload, so they're dropped here too (see the
// EventsDataTableSection port for the same pattern).
