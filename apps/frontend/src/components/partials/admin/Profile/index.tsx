...
import { fetchSelf, fetchUser, updateSelf } from "@/services/admin-user.service";
...
import ProfileNewsDataTableSection from "./ProfileNewsDataTableSection";
import ProfileNewsFilterSection from "./ProfileNewsFilterSection";
import ProfileOverviewSection from "./ProfileOverviewSection";

// Ported from apps/adminpanel's src/pages/(user)/ProfilePage/index.tsx.
// Shared between the two Next.js routes that render it:
//   - app/(dashboard)/admin/users/[id]/page.tsx  -> <Profile isUserView />
//   - app/(dashboard)/admin/user/profile/page.tsx -> <Profile />
// react-router's useParams() -> next/navigation's useParams(); the original
// always cached the fetched user under the query key ["user", "self"]
// regardless of whether it was `fetchSelf()` or `fetchUser(id)` behind it —
// harmless when only one profile is ever open at a time, but means
// navigating between two different `/admin/users/[id]` profiles without a
// full reload would show stale cached data under the same key. Fixed by
// keying the "viewing another user" case on the id instead. activeBreadcrumbs
// from useMenu() (Redux) -> handled internally by PageHeader via
// useAdminMenu()/usePathname(), so not threaded through here (same pattern
// as apps/frontend's already-ported EventsPage/CategoriesDetailsPage).
const Profile = ({ isUserView }: { isUserView?: boolean }) => {
  ...
  const {
    ...
    queryKey: isUserView ? ["user", id] : ["user", "self"],
    queryFn: isUserView ? () => fetchUser(id || "") : fetchSelf,
    enabled: isUserView ? Boolean(id) : true,
  });
